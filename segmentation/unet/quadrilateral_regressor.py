...
def focal_loss(gamma=2., alpha=.25):
    ...
    loss_func.__name__ = 'focal_loss'
    return loss_func


def smoothL1(delta=0.5):
    # https://stackoverflow.com/questions/44130871
    def loss_func(y_true, y_pred):
       x = K.abs(y_true - y_pred)
       x = K.switch(x < delta, 0.5 * x ** 2, delta * (x - 0.5 * delta))
       return  K.sum(x)
    loss_func.__name__ = 'smooth_l1_loss'
    return loss_func
...
if __name__ == '__main__':
    ...
    args.add_argument("--use_homography_proposals",
                      default=False, action='store_true',
                      help="Use homographies proposals for template "
                           "matching loss.")
    args.add_argument(
    ...
    if args.command == 'train':
        m = train(data_dir=args.input,
                  ...
                  use_homograpy_proposals=args.use_homograpy_proposals)
